...
/*	
	...
	irq_node = of_find_compatible_node(NULL, NULL, "mediatek,gyroscope");	
...
//		pinctrl_select_state(pinctrl, pins_cfg);
		...
		res = request_irq(obj->irq, icm426xx_accel_eint_handler, IRQF_TRIGGER_RISING, "icm426xx_accint", NULL);
		if(res) {
...
static const struct of_device_id accel_of_match[] = {
    { .compatible = "mediatek,gsensor_icm426xx", },
...
{	
...
#else
	const char *name = "mediatek,icm426xx_acc";
#endif	
	ACC_LOG("%s: OK\n",  __func__);
    ...
    while(ptr != NULL)
    {
        if (strcmp(ptr->name.buffer, icm426xx_accel_init_info.name) == 0)
        {
            found = 1;
            break;
        }
