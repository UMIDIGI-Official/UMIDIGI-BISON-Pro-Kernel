...
static int tekhw_imx586_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_imx586_color =0xff;
...
#ifdef BENJA_IMX586_LRC
extern unsigned int read_imx586_LRC(BYTE *data);
static void write_imx586_LRC_Data(void)
{
	...
	for (i = 192; i < LRC_SIZE; i++) {
		write_cmos_sensor(0x7600 + i-192, imx586_lrc_data[i]);
		/* LOG_INF("imx586_lrc_data[%d] = 0x%x\n", i, imx586_lrc_data[i]); */
	}
}
#endif
...
kal_uint16 addr_data_pair_lrc_imx586[] = {
 0x7510,0x6e,
 0x7511,0x70,
 0x7512,0x73,
 0x7513,0x74,
...
#if 1//def BENJA_IMX586_LRC
	//imx586_table_write_cmos_sensor(addr_data_pair_lrc_imx586,
      //   sizeof(addr_data_pair_lrc_imx586) / sizeof(kal_uint16));  
#endif
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017));
            /*LOG_INF("[getid imx586]read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",
                read_cmos_sensor_8(0x0016),
                read_cmos_sensor_8(0x0017),
                read_cmos_sensor(0x0000));*/
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[getid imx586]i2c write id: 0x%x, sensor id: 0x%x\n",
                    imgsensor.i2c_write_id, *sensor_id);

                return ERROR_NONE;
            }

            LOG_INF("[getid imx586]Read sensor id fail, id: 0x%x\n",
                imgsensor.i2c_write_id);
            retry--;
        } while (retry > 0);
        i++;
...
static kal_uint32 open(void)
{
   ...
   // kal_uint16 j = 0;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
                    | read_cmos_sensor_8(0x0017));
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[open imx586]i2c write id: 0x%x, sensor id: 0x%x\n",
                    imgsensor.i2c_write_id, sensor_id);
                break;
            }
            LOG_INF("[open imx586]Read sensor id fail, id: 0x%x\n",
                imgsensor.i2c_write_id);
            retry--;
        } while (retry > 0);
        i++;
	...
	sensor_init();
	imx586_table_write_cmos_sensor(addr_data_pair_lrc_imx586,sizeof(addr_data_pair_lrc_imx586) / sizeof(kal_uint16));  
#ifdef BENJA_IMX586_LRC	
	read_imx586_LRC(imx586_lrc_data);
	LOG_INF("[%s][%d]imx586_lrc_data[0]:0x%x   imx586_lrc_data[383]:0x%x\n", __func__, __LINE__,imx586_lrc_data[0],imx586_lrc_data[383]);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_imx586_mirror)
    {
	    set_mirror_flip(tekhw_imx586_mirror);
    }
#else
...
{
    tekhw_camera_data* tekhw_imx586_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_IMX586_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_imx586_ptr != NULL) && (tekhw_imx586_ptr->set_mirror.size > 0))
    {
        tekhw_imx586_mirror = *(tekhw_u16*)tekhw_imx586_ptr->set_mirror.buffer;
    }
    if ((tekhw_imx586_ptr != NULL) && (tekhw_imx586_ptr->set_color.size > 0))
    {
		tekhw_imx586_color= *(tekhw_u16*)tekhw_imx586_ptr->set_color.buffer;
    }
}
#endif
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(tekhw_imx586_color!=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_imx586_color;
	}
	tekhw_times =1;
#endif
