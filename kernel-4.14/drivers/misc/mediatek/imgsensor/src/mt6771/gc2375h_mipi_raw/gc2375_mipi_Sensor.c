...
static int tekhw_gc2375h_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_gc2375h_color =0xff;
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0xf7375923, //0xf7375923,    /* checksum value for Camera Auto Test */
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
	    ...
	    do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("gc2375h open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("gc2375h open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
	    } while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_gc2375h_mirror)
    {
	    set_mirror_flip(tekhw_gc2375h_mirror);
    }
#endif
	return ERROR_NONE;
}
...
{
    tekhw_camera_data* tekhw_gc2375h_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_GC2375H_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_gc2375h_ptr != NULL) && (tekhw_gc2375h_ptr->set_mirror.size > 0))
    {
        tekhw_gc2375h_mirror = *(tekhw_u16*)tekhw_gc2375h_ptr->set_mirror.buffer;
    }
    if ((tekhw_gc2375h_ptr != NULL) && (tekhw_gc2375h_ptr->set_color.size > 0))
    {
		tekhw_gc2375h_color= *(tekhw_u16*)tekhw_gc2375h_ptr->set_color.buffer;
    }
}
#endif
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(tekhw_gc2375h_color !=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_gc2375h_color;
	}
	tekhw_times=1;
#endif
