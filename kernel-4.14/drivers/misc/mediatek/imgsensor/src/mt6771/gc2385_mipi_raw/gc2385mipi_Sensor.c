...
static int tekhw_gc2385_mirror = 0xff;/*add by xunhu andy andy20160720 at 22:48*/
static int tekhw_gc2385_color =0xff;
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .linelength = 1189,    //1368 sjs            //record different mode's linelength
        .framelength = 1437,  //1248 sjs          //record different mode's framelength
	...
	.custom2 = {
        .pclk = 32300000,                //record different mode's pclk
	...
	.frame_time_delay_frame = 1,//  The delay frame of setting frame length dual camera
...
{	
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 1.99x , GC2385MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 2.86x , GC2385MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 4.01x , GC2385MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 5.76x , GC2385MIPI add pregain = %d\n",temp);
	}	
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 8.30x , GC2385MIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_8<= iReg)&&(iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 11.72x , GC2385MIPI add pregain = %d\n",temp);
	}	
	...
	else 
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MIPI analogic gain 17.06x , GC2385MIPI add pregain = %d\n",temp);
	}
	return gain;
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	{
			case IMAGE_NORMAL:
						write_cmos_sensor(0x17,0xd4);write_cmos_sensor(0x92,0x03);write_cmos_sensor(0x94,0x05); 
						write_cmos_sensor(0x4e,0x3c);write_cmos_sensor(0x4f,0x00);
						break;
			case IMAGE_H_MIRROR:
						write_cmos_sensor(0x17,0xd5);write_cmos_sensor(0x92,0x03);write_cmos_sensor(0x94,0x06); 
						write_cmos_sensor(0x4e,0x3c);write_cmos_sensor(0x4f,0x00);
						break;
			case IMAGE_V_MIRROR:
						write_cmos_sensor(0x17,0xd6);write_cmos_sensor(0x92,0x02);write_cmos_sensor(0x94,0x05); 
						write_cmos_sensor(0x4e,0x00);write_cmos_sensor(0x4f,0x3c);
						break;
			case IMAGE_HV_MIRROR:
						write_cmos_sensor(0x17,0xd7);write_cmos_sensor(0x92,0x02);write_cmos_sensor(0x94,0x06); 
						write_cmos_sensor(0x4e,0x00);write_cmos_sensor(0x4f,0x3c);
						break;
			default :
			            break;		
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x05,0x02);// 02 sjs
	write_cmos_sensor(0x06,0xf4);//86 sjs
	write_cmos_sensor(0x07,0x00);//00 sjs
	write_cmos_sensor(0x08,0xd4);//10 sjs
	write_cmos_sensor(0x09,0x00); 
	write_cmos_sensor(0x0a,0x04); 
	write_cmos_sensor(0x0b,0x00); 
	write_cmos_sensor(0x0c,0x02); 
#ifndef XUNHU_LPS_TEKHW_SUPPORT
        ...
        do {
            *sensor_id = return_sensor_id()&0xffef;
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[gc2385] Read sensor id ok ,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("[gc2385] Read sensor id fail, write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()&0xffef;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("[gc2385]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("[gc2385]Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    preview_setting();
#ifdef XUNHU_LPS_TEKHW_SUPPORT
    if (0xff != tekhw_gc2385_mirror)
    {
	    set_mirror_flip(tekhw_gc2385_mirror);
    }
#endif
    return ERROR_NONE;
}    /*    preview   */
...
{
    tekhw_camera_data* tekhw_gc2385_ptr = tekhw_get_appointed_camera_ptr(SENSOR_DRVNAME_GC2385_MIPI_RAW);/*add by xunhu andy andy20160720 at 22:56*/
    if ((tekhw_gc2385_ptr != NULL) && (tekhw_gc2385_ptr->set_mirror.size > 0))
    {
        tekhw_gc2385_mirror = *(tekhw_u16*)tekhw_gc2385_ptr->set_mirror.buffer;
    }
    if ((tekhw_gc2385_ptr != NULL) && (tekhw_gc2385_ptr->set_color.size > 0))
    {
		tekhw_gc2385_color= *(tekhw_u16*)tekhw_gc2385_ptr->set_color.buffer;
    }
}
#endif
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(tekhw_gc2385_color !=0xff)
	{
		sensor_info->SensorOutputDataFormat = tekhw_gc2385_color;
	}
	tekhw_times=1;
#endif
