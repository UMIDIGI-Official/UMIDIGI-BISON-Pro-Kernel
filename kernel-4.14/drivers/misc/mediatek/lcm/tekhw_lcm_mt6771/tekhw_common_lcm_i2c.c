...
int tekhw_lcm_i2c_write_bytes(unsigned char addr, unsigned char value)
{
	...
	char write_data[2] = { 0 };
	pr_debug("lcm_i2c_addr=%x\n",addr);
	pr_debug("lcm_i2c_value=%x\n",value);
	...
	pr_debug("lcm_i2c_ret=%d\n",ret);
	if (ret < 0)
		printk("[LCM][ERROR] _lcm_i2c write data fail !!\n");

	return ret;
}
EXPORT_SYMBOL(tekhw_lcm_i2c_write_bytes);
