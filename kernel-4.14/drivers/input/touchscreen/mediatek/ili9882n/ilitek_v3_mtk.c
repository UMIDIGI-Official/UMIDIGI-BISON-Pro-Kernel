...
#define DTS_OF_NAME	"mediatek,cap_touch_ili9882n"
...
static char *ilitek_9882n_get_tp_firmware_version(void)
{
	int ret=0;
	u8 buf[10] = {0};
	u8 cmd[2] = {0};
	...
	ret = ilits->wrapper(&cmd[1],sizeof(u8), buf, sizeof(buf), OFF, OFF);

	ILI_INFO(" buf0=%d,buf1=%d,buf2=%d,buf3=%d,buf4=%d, \n",buf[0],buf[1],buf[2],buf[3],buf[4]);
	...
	{
		snprintf(firmware_version, sizeof(firmware_version), "Tpfirmware version:%d.%d.%d.%d\n",buf[1],buf[2],buf[3],buf[4]);
	}
...
static struct tpd_driver_t tpd_device_driver = {
	...
	.suspend = tpd_suspend,
	.resume = tpd_resume,
	.get_tp_firmware_version=ilitek_9882n_get_tp_firmware_version,
};
